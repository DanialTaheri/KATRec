...
class KGAT_loader(Data):
    ...
    def _get_relational_adj_list(self):
        ...
        adj_mat_list = []
        adj_r_list = []
        # np_mat is combination of [user1, items1], [user1, item2], ..., [user2, item1],...
        def _np_mat2sp_adj(np_mat, row_pre, col_pre):
            n_all = self.n_users + self.n_entities
            # single-direction
            a_rows = np_mat[:, 0] + row_pre - 1
    ...
    # generate users, pos_items, and neg_items from the load_data file. Some important points:
    #1) each batch includes bunch of users without repetition
    def generate_train_batch(self):
        users, pos_items, neg_items = self._generate_train_cf_batch()
        print("positive items shape", len(pos_items))
        print("user shape", users)
        batch_data = {}
    ...
    def generate_train_feed_dict(self, model, batch_data):
        feed_dict = {
            ...
            model.mess_dropout: eval(self.kwargs['mess_dropout']),
            model.node_dropout: eval(self.kwargs['node_dropout']),
        }

        return feed_dict
        # gets the train relations: head, tails and positive tails
    ...
    def generate_test_feed_dict(self, model, user_batch, item_batch, drop_flag=True):
        ...
        feed_dict ={
            ...
            model.mess_dropout: [0.] * len(eval(self.kwargs['layer_size'])),
            model.node_dropout: [0.] * len(eval(self.kwargs['layer_size'])),
