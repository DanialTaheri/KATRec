...
class BertConfig():
    ...
    # classmethod works by calling for instance BertConfig.from_dict(jsonfile)
    # On the other hand staticmethod works like a function and there is not dependency between that and the class, it is used just for the purpose of organization for instance: BertConfig.from_dict(value)
    @classmethod
...
class KATRecModel(object):
    ...
    ```
    """
    # config is the configuration of the bert and data_config is the KG configuration
    def __init__(self,
                 head, 
                 relation, 
                 pos_t, 
        ...
        self._parse_args(data_config, pretrain_data, FLAGS)
        config = copy.deepcopy(config)
        if not is_training:
            config.hidden_dropout_prob = 0.0
            config.attention_probs_dropout_prob = 0.0
        self.n_items = config.vocab_size
        ...
        # creating a graph for Kgat and bert
        with tf.variable_scope(scope, default_name="kATRec"):
            
            self.weights = self._build_weights()
            with tf.variable_scope('Spacial'):
                ####Loss 1
                # embeddings of head, positive tail embedding and negative tail embedding
                h_e, r_e, pos_t_e, neg_t_e = self._get_kg_inference(head, relation, pos_t, neg_t)
                # knowledge-aware attention score pi(h,r,t)
                ...
                kg_reg_loss = tf.nn.l2_loss(h_e) + tf.nn.l2_loss(r_e) + \
                tf.nn.l2_loss(pos_t_e) + tf.nn.l2_loss(neg_t_e)
            ...
            with tf.variable_scope('Sequential'):
                ...
                if FLAGS.use_KG_attention:
                    self.update_attentive_A() 
                    
                with tf.variable_scope("embeddings"):
                # Perform embedding lookup on the word ids.
                    self.user_embedding = user_embedding_lookup(
                        user_ids=user_ids,
                        user_embedding=self.ua_embeddings,
                        embedding_size=config.hidden_size)
                                 
                    (self.embedding_output,
                     self.embedding_table) = embedding_lookup(
                         input_ids=input_ids,
                         ea_embeddings=self.ea_embeddings,
                         vocab_size=config.vocab_size,
                         embedding_size=config.hidden_size,
                         initializer_range=config.initializer_range,
                         word_embedding_name="word_embeddings",
                         use_one_hot_embeddings=use_one_hot_embeddings,
                         use_KG_embedding=FLAGS.use_KG_connection)
                ...
                # for the attention scores.
                    ...
                    self.all_encoder_layers, self.attention_weights = transformer_model(
                        input_tensor=self.embedding_output,
    ...
    def _parse_args(self, data_config, pretrain_data, FLAGS):
        # argument settings
        ...
        self.n_entities = data_config['n_entities']#item+other
        self.n_relations = data_config['n_relations']
        self.n_fold = 100 #change this later
        ...
        self.adj_uni_type = FLAGS.adj_uni_type

        self.lr = FLAGS.lr
        
        # settings for CF part.
        self.emb_dim = FLAGS.embed_size
        #print("embedding dimension", self.emb_dim)
        self.batch_size = FLAGS.batch_size

        # settings for KG part.
        self.kge_dim = FLAGS.kge_size
        self.batch_size_kg = FLAGS.batch_size_kg

        self.weight_size = eval(FLAGS.layer_size)
        self.n_layers = len(self.weight_size)

        self.alg_type = FLAGS.alg_type

        self.regs = eval(FLAGS.regs)
        self.mess_dropout = eval(FLAGS.mess_dropout)
    ...
    def _build_weights(self):
        ...
        if self.pretrain_data is None:
            ...
            if self.pretrain_data['user_embed'].shape[0] != self.n_users:
                all_weights['user_embed'] = tf.Variable(initializer([self.n_users, self.emb_dim]), name='user_embed')
            else:
            ...
            print(all_weights['user_embed'])

            item_embed = self.pretrain_data['item_embed']
            # subtracting "MASK", "PAD", ...
            temp_n_items = self.n_items - 4
            other_embed = initializer([self.n_entities - temp_n_items, self.emb_dim])
    ...
    def get_kg_embedding_table(self):
        return self.ea_embeddings[:self.n_items, :]
    
    def get_kg_user_embedding(self):
    ...
    def _create_bi_interaction_embed(self):
        ...
        #print("Shape of ego_embedding", ego_embeddings.get_shape())
        ...
        for k in range(0, self.n_layers):
            ...
            #print("Shape of norm_embedding", norm_embeddings.get_shape())
            all_embeddings += [norm_embeddings]

        all_embeddings = tf.concat(all_embeddings, 1)
        #print("Shape of all_embeddings", all_embeddings.get_shape())
    ...
    def update_attentive_A(self):
        ...
        for i_fold in range(self.n_fold):
            ...
            if i_fold == self.n_fold - 1:
                ...
                """
                 ...
                 """
            A_kg_score = self._generate_transE_score(self.all_h_list[start:end],self.all_t_list[start:end],self.all_r_list[start:end])

            #print(A_kg_score.get_shape(), "A_kg_score.get_shape()")
            
            #kg_score += list(A_kg_score) #change and debug this
            kg_score = tf.concat([kg_score, A_kg_score], 0)
            #print(kg_score.get_shape(), "kg_score.get_shape()")

        #kg_score = np.array(kg_score)
        #print(type(kg_score), "type(kg_score)")
        #print(kg_score.get_shape(), "kg_score.get_shape()")
        ...
        #print("type(new_A_indices)", type(new_A_indices))
        #print("new_A_indices.get_shape()", new_A_indices.get_shape())
        
        rows = new_A_indices[:, 0]
        #print("type(rows)", type(rows))
        #print("rows.get_shape()", rows.get_shape())
        ...
        self.A_in = tf.SparseTensorValue(indices=(rows, cols), values=new_A_values,dense_shape=(self.n_users + self.n_entities, self.n_users + self.n_entities))
...
def user_embedding_lookup(user_ids,
                          user_embedding, 
                          embedding_size):
    if user_ids.shape.ndims == 2:
        user_ids = tf.expand_dims(user_ids, axis=[-1])

    output = tf.nn.embedding_lookup(user_embedding, user_ids)

    input_shape = get_shape_list(user_ids)
...
def embedding_lookup(input_ids,
    ...
    #embeddings of non-items embeddings
    if use_KG_embedding:
        ea_nonitems = tf.get_variable(
            name = word_embedding_name,
            shape = [4, embedding_size],
            initializer=create_initializer(initializer_range))

        embedding_table = tf.concat([ea_embeddings[:vocab_size-4, :], ea_nonitems], axis = 0)
    else:
    ...
    if use_one_hot_embeddings:
        ...
        #one hote input ids = [batchsize, seq_length, vocab_size]
        one_hot_input_ids = tf.one_hot(flat_input_ids, depth=vocab_size)
        output = tf.matmul(one_hot_input_ids, embedding_table)
    else:
        #print("embedding_table, input_ids", ea_embeddings.get_shape(), input_ids.get_shape())
...
def transformer_model(input_tensor,
        ...
        with tf.variable_scope("layer_%d" % layer_idx):
            ...
            with tf.variable_scope("attention"):
                ...
                with tf.variable_scope("self"):
                    ...
                    att_maps.append(attention_probs)
    ...
    att_maps = tf.stack(att_maps, 1)
    if do_return_all_layers:
        final_outputs = []
        for layer_output in all_layer_outputs:
            final_output = reshape_from_matrix(layer_output, input_shape)
            final_outputs.append(final_output)
        return final_outputs, att_maps
    else:
        final_output = reshape_from_matrix(prev_output, input_shape)
        return final_output, att_maps
