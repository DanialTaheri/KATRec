...
def read_KGAT_text(FLAGS):
    if FLAGS.model_type == 'bprmf':
        data_generator = BPRMF_loader(FLAGS=FLAGS, path=FLAGS.data_path_kgat + FLAGS.dataset_name)
        batch_test_flag = False

    elif FLAGS.model_type == 'cke':
        data_generator = CKE_loader(FLAGS=FLAGS, path=FLAGS.data_path_kgat + FLAGS.dataset_name)
        batch_test_flag = False

    elif FLAGS.model_type in ['cfkg']:
        data_generator = CFKG_loader(FLAGS=FLAGS, path=FLAGS.data_path_kgat + FLAGS.dataset_name)
        batch_test_flag = True

    elif FLAGS.model_type in ['fm','nfm']:
        data_generator = NFM_loader(FLAGS=FLAGS, path=FLAGS.data_path_kgat + FLAGS.dataset_name)
        batch_test_flag = True

    elif FLAGS.model_type in ['kgat']:

        data_generator = KGAT_loader(FLAGS=FLAGS, path=FLAGS.data_path_kgat + FLAGS.dataset_name)
...
def write_instance_to_example_files(instances, max_seq_length,
    ...
    total_written = 0
    FLAGS.n_instances = len(instances)
    if training:
        data_generator = read_KGAT_text(FLAGS)
    ...
    for (inst_index, instance) in enumerate(instances):
        ...
        if training:
            batch_data = data_generator.generate_train_A_batch()
            features["h"] = create_int_feature(batch_data["heads"])
            features["r"] = create_int_feature(batch_data["relations"])
            features["pos_t"] = create_int_feature(batch_data["pos_tails"])
            features["neg_t"] = create_int_feature(batch_data["neg_tails"])
...
def gen_samples(data,
    ...
    print("data_path_kgat", FLAGS.data_path_kgat)
    path = FLAGS.data_path_kgat + FLAGS.dataset_name
    print("path", path)
    bert_output = []
    #Generating KGAT files
    for instance in instances:
        bert_output.append(" ".join([printable_text(x) for x in instance.tokens]))
    #greatres train and test txt inpute files for KGAT.  
    #generate_KGAT_txt_inputs(bert_output, path, train) 
    


    write_instance_to_example_files(instances, max_seq_length,
                                    max_predictions_per_seq, vocab,
                                    [output_filename], FLAGS, train)
    
# def generate_KGAT_txt_inputs(bert_output, path, train):
#     if train:
#         types = 'train'
#     else:
#         types = 'test'
#     print("train or test??", types)
#     path += "/" + types + ".txt"
#     for i in range(0,len(bert_output)):
#         line = bert_output[i].strip()
...
def main():
    ...
    dataset = data_partition(output_dir+dataset_name+'.txt', FLAGS)
...
#             print(user_test[u])
    ...
    # this should be removed in final test
#     user_valid_data = {
#         'user_' + str(u):
...
#     }
    ...
    # this should be removed in the final test 
#     vocab = FreqVocab(user_valid_data)
...
#     }
    ...
    # This shoudl be removed in the final test
#     print('begin to generate valid')
#     output_filename = output_dir + dataset_name + version_id + '.valid.tfrecord'
#     gen_samples(
#         user_valid_data,
#         output_filename,
#         rng,
